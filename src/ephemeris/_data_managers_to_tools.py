...
def read_data_managers_configuration(path: str) -> Dict[str, DataManager]:
    with open(path, "r") as f:
        raw_data_manager_conf = yaml.safe_load(f)
    data_managers = {}
    for repository_name, data_manager_configuration in raw_data_manager_conf.items():
        data_manager = DataManager(
            tool_id=data_manager_configuration["tool_id"],
            repository_name=repository_name,
            tags=data_manager_configuration.get("tags") or [],
        )
